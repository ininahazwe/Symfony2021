...
export default function checkUserIpEntered(event){
    ...
    if (pressed_key === 'enter'){
        ...
        alert("L'adresse IP doit se terminer par un caractère hexadécimal : 0 à 9 ou A à F (miniscule ou majuscule).");
        throw new Error("L'adresse IP doit se terminer par un caractère hexadécimal: 0 à 9 ou A à F (miniscule ou majuscule).");
        }
        ...
        if (JSON.stringify(ip_addresses) !== JSON.stringify([""])){
            ip_addresses.forEach(ip_addresses => {
                if (!ipv4_and_ipv6_regex.test(ip_addresses)){
                    alert(`L'adresse Ip suivante n'est pas valide : ${ip_addresses}`);
                    throw new Error(`L'adresse IP suivante n'est pas valide : ${ip_addresses}`);
                }
            });
        }
